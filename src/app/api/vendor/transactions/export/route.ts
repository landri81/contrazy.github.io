...
import {
  getVendorTransactionsCsvRows,
  getVendorTransactionsExportAvailableRange,
  type VendorTransactionsCsvRow,
} from "@/features/dashboard/server/vendor-transactions-export"
import {
...
const csvColumns: CsvColumn<VendorTransactionsCsvRow>[] = [
...
export async function POST(request: Request) {
  try {
    ...
    const { q, status, kind, startDate, endDate } = payload.data

    if (startDate > endDate) {
      return jsonExportError("Start date cannot be after end date.", 422)
    }

    const availableRange = await getVendorTransactionsExportAvailableRange(vendorProfile.id, {
      q,
      status,
      kind,
    })
    ...
    const rows = await getVendorTransactionsCsvRows(vendorProfile.id, {
      q,
      status,
      kind,
      startDate,
    ...
    return createCsvDownloadResponse(csv, "vendor-transactions")
  } catch (error) {
    console.error("Vendor transactions export failed", error)
    return jsonExportError("Failed to export vendor transactions.", 500)
  }
}
